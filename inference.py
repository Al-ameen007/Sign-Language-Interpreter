...
from preprocessing import sample_frames, detect_landmarks_mediapipe, normalize_landmarks, construct_graph, get_adjacency_matrix
...
def predict_live_webcam(model, label_encoder, device='cpu', buffer_size=48, display=True):
    ...
    while True:
        ...
        if display:
            ...
            cv2.putText(display_frame, f"Prediction: {last_prediction}", (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            
            status_color = (0, 255, 0) if len(frame_buffer) == buffer_size else (0, 165, 255)
            cv2.putText(display_frame, "READY" if len(frame_buffer) == buffer_size else "BUFFERING...", 
