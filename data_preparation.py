...
def verify_combined_folder(combined_path, folder_type="train/test"):
    ...
    if video_count == 0:
        ...
        print(f"     Check that your source folders contain video files!")
        return False
...
def prepare_karsl_simple(base_path, labels_file, output_csv_dir='csvs', 
                        force_recombine=False, skip_train=False, skip_test=False):
    print("="*60)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--base-path', type=str, required=True,
                       help='Path to folder containing 01, 02, 03 folders')
    parser.add_argument('--labels-file', type=str, required=True,
                       help='Path to KARSL-502_Labels.xlsx')
    parser.add_argument('--csv-dir', type=str, default='csvs',
                       help='Output directory for CSV files')
    parser.add_argument('--force-recombine', action='store_true',
                       help='Force recombining folders even if they already exist')
    parser.add_argument('--skip-train', action='store_true',
                       help='Skip train processing (use existing train.csv)')
    parser.add_argument('--skip-test', action='store_true',
                       help='Skip test processing (use existing test.csv)')
